goog.module('rxjs$operator$switchMap');
 ...
 * @example <caption>Rerun an interval Observable on every click event</caption> var clicks = Rx.Observable.fromEvent(document, 'click'); var result = clicks.switchMap((ev) => Rx.Observable.interval(1000)); result.subscribe(x => console.log(x)); *
 * @see {@link concatMap}
 * @see {@link exhaustMap}
 * @see {@link mergeMap}
 * @see {@link switch}
 * @see {@link switchMapTo} * that, when applied to an item emitted by the source Observable, returns an Observable. A function to produce the value on the output Observable based on the values and the indices of the source (outer) emission and the inner Observable emission. The arguments passed to this function are: - `outerValue`: the value that came from the source - `innerValue`: the value that came from the projected Observable - `outerIndex`: the "index" of the value that came from the source - `innerIndex`: the "index" of the value from the projected Observable projection function (and the optional `resultSelector`) to each item emitted by the source Observable and taking only the values from the most recently projected inner Observable.
 * @method switchMap
 * @owner Observable
 * @param {?} project
...
class SwitchMapOperator {
    ...
    static _tsickle_typeAnnotationsHelper() {
        /** @type {?} */
        SwitchMapOperator.prototype.project;
        /** @type {?} */
        SwitchMapOperator.prototype.resultSelector;
    }
}
/**
...
class SwitchMapSubscriber extends OuterSubscriber_1.OuterSubscriber {
    ...
    static _tsickle_typeAnnotationsHelper() {
        ...
        /** @type {?} */
        SwitchMapSubscriber.prototype.project;
        /** @type {?} */
        SwitchMapSubscriber.prototype.resultSelector;
    }
}
