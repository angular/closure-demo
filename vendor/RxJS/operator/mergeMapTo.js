goog.module('rxjs$operator$mergeMapTo');
 ...
 * @example <caption>For each click event, start an interval Observable ticking every 1 second</caption> var clicks = Rx.Observable.fromEvent(document, 'click'); var result = clicks.mergeMapTo(Rx.Observable.interval(1000)); result.subscribe(x => console.log(x)); *
 * @see {@link concatMapTo}
 * @see {@link merge}
 * @see {@link mergeAll}
 * @see {@link mergeMap}
 * @see {@link mergeScan}
 * @see {@link switchMapTo} * the source Observable. A function to produce the value on the output Observable based on the values and the indices of the source (outer) emission and the inner Observable emission. The arguments passed to this function are: - `outerValue`: the value that came from the source - `innerValue`: the value that came from the projected Observable - `outerIndex`: the "index" of the value that came from the source - `innerIndex`: the "index" of the value from the projected Observable Observables being subscribed to concurrently. `innerObservable` (and optionally transformed through `resultSelector`) every time a value is emitted on the source Observable.
 * @method mergeMapTo
 * @owner Observable
...
class MergeMapToOperator {
    ...
    static _tsickle_typeAnnotationsHelper() {
        /** @type {?} */
        MergeMapToOperator.prototype.ish;
        /** @type {?} */
        MergeMapToOperator.prototype.resultSelector;
        /** @type {?} */
        MergeMapToOperator.prototype.concurrent;
    }
}
...
class MergeMapToSubscriber extends OuterSubscriber_1.OuterSubscriber {
    ...
    static _tsickle_typeAnnotationsHelper() {
        ...
        /** @type {?} */
        MergeMapToSubscriber.prototype.ish;
        /** @type {?} */
        MergeMapToSubscriber.prototype.resultSelector;
        /** @type {?} */
        MergeMapToSubscriber.prototype.concurrent;
    }
}
